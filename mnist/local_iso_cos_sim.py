...
class ContrastiveLearningTransform:
    def __init__(self, base_rot_range, num_nn, local_rot_range):
    ...
    def __call__(self, x):
        ...
        x = T.Resize(87)(x)
        base_rot_angle = random.random() * self.base_rot_range
        rotate_angles = [base_rot_angle + random.random() * self.local_rot_range for _ in range(self.num_nn)]
        xs = [
...
def ssl_loop(args, encoder=None):
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset=MnistRotDataset(
            ...
            transform=ContrastiveLearningTransform(args.base_rot_range, args.num_nn, args.local_rot_range)
        ),
        shuffle=True,
        batch_size=args.bsz,
        pin_memory=True,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--local_rot_range', default=10, type=float)
