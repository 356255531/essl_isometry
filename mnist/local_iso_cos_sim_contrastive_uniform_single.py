...
class ContrastiveLearningTransform:
    ...
    def __call__(self, x):
        ...
        x = T.Resize(87)(x)
        base_rot = random.random() * self.base_rot_range / self.num_base_rotation
        base_rot_angles = [(i / self.num_base_rotation) * self.base_rot_range + base_rot for i in range(self.num_base_rotation)]
        rotate_angles = [
...
def cos_sim_loss(z, angles):
    ...
    fig = plt.figure()
    coords = z[0][0].detach().cpu().numpy()
    plt.scatter(coords[:, 0], coords[:, 1], c='b')
    coords_gt = angles[0][0].detach().cpu().numpy() * np.pi / 180
...
def ssl_loop(args):
    ...
    for e in range(1, args.epochs + 1):
        ...
        line_to_print = (
            f'epoch: {e} | '
            f'loss: {loss:.3f} | '
            f'equi contrastive loss: {equi_contrastive_loss:.3f} | '
            f'equi iso loss: {equi_iso_loss:.3f} | '            
            f'lr: {lr:.6f} | '
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_dir', default='experiment/LocalIsoCosSimContrastiveUniformSingle/', type=str)
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--lmbd', default=0.1, type=float)
    parser.add_argument('--num_base_rot', default=36, type=int)
    parser.add_argument('--base_rot_range', default=360, type=int)
    parser.add_argument('--num_nn', default=8, type=int)
    parser.add_argument('--local_rot_range', default=360, type=int)
    parser.add_argument('--num_workers', default=8, type=int)
    parser.add_argument('--checkpoint_path', default="experiment/LocalIsoCosSimContrastiveUniformSingle/800.pth", type=str)
